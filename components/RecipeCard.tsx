...
export default function RecipeCard({ recipe, title }: RecipeCardProps) {
  ...
  const { prep_time_minutes, cook_time_minutes, total_time_minutes, yield_amount, yield_unit, difficulty, cuisine, ingredients, instructions, tips } = recipe

  return (
    <div className="rounded-2xl overflow-hidden mb-12"
      style={{ border: '1px solid var(--color-bg-secondary)', backgroundColor: 'var(--color-bg-primary)' }}>
      ...
      {(difficulty || cuisine) && (
        <div className="flex flex-wrap gap-2 px-6 pt-4">
          {difficulty && <span className="px-3 py-1 rounded-full text-xs font-semibold"
            style={{ backgroundColor: 'var(--color-bg-secondary)', color: 'var(--color-text-secondary)' }}>{difficulty}</span>}
          {cuisine && <span className="px-3 py-1 rounded-full text-xs font-semibold"
            style={{ backgroundColor: 'var(--color-bg-secondary)', color: 'var(--color-text-secondary)' }}>{cuisine}</span>}
        </div>
      )}
      ...
      <div className="px-6 py-6 space-y-8">
        ...
        {ingredients && ingredients.length > 0 && (
          <div>
            <h2 className="text-xl font-bold mb-4"
              style={{ color: 'var(--color-text-primary)', fontFamily: 'var(--font-heading)' }}>Ingredients</h2>
            <ul className="space-y-2">
              {ingredients.map((ingredient, i) => (
                <li key={i} className="flex items-start gap-3 cursor-pointer group" onClick={() => toggleIngredient(i)}>
        ...
        {ingredients?.length && instructions?.length ? <hr style={{ borderColor: 'var(--color-bg-secondary)' }} /> : null}

        {/* Instructions */}
        {instructions && instructions.length > 0 && (
          <div>
            <h2 className="text-xl font-bold mb-4"
              style={{ color: 'var(--color-text-primary)', fontFamily: 'var(--font-heading)' }}>Instructions</h2>
              ...
              {instructions.map((step, i) => (
                <li key={i} className="flex gap-4">
                  ...
                  <div className="flex-1 pt-1">
                    {step.name && <p className="font-semibold mb-1 text-sm" style={{ color: 'var(--color-text-primary)' }}>{step.name}</p>}
        ...
        {tips && tips.length > 0 && (
          <>
            ...
            <div>
              <h2 className="text-xl font-bold mb-4"
                style={{ color: 'var(--color-text-primary)', fontFamily: 'var(--font-heading)' }}>Tips & Notes</h2>
              <ul className="space-y-2">
                {tips.map((tip, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm leading-relaxed" style={{ color: 'var(--color-text-primary)' }}>
                    <span style={{ color: 'var(--color-accent)', marginTop: '0.125rem' }} className="flex-shrink-0">✦</span>
                    <span>{tip}</span>
                  </li>
                ))}
              </ul>
